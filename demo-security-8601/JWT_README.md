...
<dependency>
    ...
    <artifactId>java-com.djun.demo.jwt</artifactId>
...
```gradle
implementation 'com.auth0:java-com.djun.demo.jwt:3.8.1'
```
...
try {
    ...
    DecodedJWT com.djun.demo.jwt = verifier.verify(token);
...
String token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXUyJ9.eyJpc3MiOiJhdXRoMCJ9.AbIJTDMFc7yUa5MhvcP03nJPyCPzZtQcGEp-zWfOkEE";
try {
    DecodedJWT com.djun.demo.jwt = JWT.decode(token);
...
```java
String algorithm = com.djun.demo.jwt.getAlgorithm();
```
...
```java
String type = com.djun.demo.jwt.getType();
```
...
```java
String contentType = com.djun.demo.jwt.getContentType();
```
...
```java
String keyId = com.djun.demo.jwt.getKeyId();
```
...
```java
Claim claim = com.djun.demo.jwt.getHeaderClaim("owner");
```
...
```java
String issuer = com.djun.demo.jwt.getIssuer();
```
...
```java
String subject = com.djun.demo.jwt.getSubject();
```
...
```java
List<String> audience = com.djun.demo.jwt.getAudience();
```
...
```java
Date expiresAt = com.djun.demo.jwt.getExpiresAt();
```
...
```java
Date notBefore = com.djun.demo.jwt.getNotBefore();
```
...
```java
Date issuedAt = com.djun.demo.jwt.getIssuedAt();
```
...
```java
String id = com.djun.demo.jwt.getId();
```
...
```java
Map<String, Claim> claims = com.djun.demo.jwt.getClaims();    //Key is the Claim name
...
```java
Claim claim = com.djun.demo.jwt.getClaim("isAdmin");
```
...
JWTVerifier verifier = JWT.require(algorithm)
    ...
    .build();
DecodedJWT com.djun.demo.jwt = verifier.verify("my.com.djun.demo.jwt.token");
```
